...
/**
 * Represents a meteor object within the game field. Meteors are elements that pose a threat to the
 * player's ship.
 */
public class Meteor {
  ...
  private void computeMeteor(int positionX, int positionY) {
    this.meteorBox = new Rect(positionX - METEOR_SIZE / 2, positionY - METEOR_SIZE / 2,
        positionX + METEOR_SIZE / 2,
        positionY + METEOR_SIZE / 2);
    //Log.d(getClass().getSimpleName(), meteorBox.toString());
  }

  /**
   * @param meteor Implementation details to check if the provided rectangle overlaps with this
   *               meteor's rectangle.
